...
class Student():
    ...
    def write_programs(self, optional=1):
    	self.num_programs= self.num_programs + optional
...
student_tups_list=[]
for each in student_tups:
	student_tups_list.append(each)
...
programmers= [Student(student_tups[0], student_tups[1], student_tups[2]) for student_tups in student_tups_list]
...
names_with_not_too_much_seniority= [new.name for new in filter(lambda x: len(x.name) > x.years_UM, programmers)]
...
def readfiles(filenames):
	...
	for each_file in filenames:
		...
		file1.close()
		list_of_str.extend(file2)
	return list_of_str

# Define len_check
def len_check(filenames1):
	gen1=[]
	for each  in filenames1:
		if len(each) > 40:
			gen1.append(each)
	return gen1
...
def main_filterer(filenames):
	gen2=[]
	file3= readfiles(filenames)
	file4= len_check(file3)
	gen2.extend(file4)
	return gen2
